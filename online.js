...
function receive(d){if(d.type==="welcome")myPlayerId=d.playerId;if(d.type==="lobby"){onlineState={lobby:d.lobby};drawLobby()}if(d.type==="state"){myPlayerId=d.playerId||myPlayerId;onlineState=window.DurakMultiplayerSession.localizeReactionForClient(d.state);onlineActive=true;document.body.classList.add("online-active");ui("onlineLobby").hidden=true;renderOnline()}if(d.type==="error"||d.type==="kicked"){joinRejected=true;status(d.message,true)}}
...
function canTransferOnline(s,p,c){if(s.phase!=="defend"||p.id!==s.defender||!s.battle.length||s.battle.some(pair=>pair.defense)||s.battle.length>=s.limit)return false;if(!s.battle.every(pair=>pair.attack.rank===c.rank))return false;let newDefender=next(s.defender),target=playerById(newDefender);return newDefender!==s.defender&&!!target&&!target.out&&target.hand.length>=s.battle.length+1}

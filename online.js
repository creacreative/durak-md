...
ui("hostRoom").onclick=()=>{isHost=true;room=code();peer=ensurePeer(`durak-${room.toLowerCase()}`);if(!peer)return;ui("lobbyChoices").hidden=true;ui("publicArea").hidden=true;ui("roomArea").hidden=false;ui("copyCode").textContent=room;peer.on("open",async()=>{connections=[];nextPlayerNumber=1;myPlayerId="player-0";onlineState={lobby:[{id:myPlayerId,name:ui("onlineName").value.trim()||"Gazdă",host:true}]};drawLobby();status("Camera este gata. Trimite codul unui prieten.");if(publicHosting){try{await window.DurakAuth.createRoom(room,`Masa lui ${onlineState.lobby[0].name}`);status("Masa publică este vizibilă. Așteaptă jucători.")}catch(e){publicHosting=false;status(e.message,true)}}});peer.on("connection",acceptConnection);peer.on("error",e=>status(`Eroare: ${e.type}`,true))};
